...
<body class="min-h-screen w-screen flex items-center justify-center">
    <main class="flex flex-col items-center justify-center border border-slate-700 bg-gray-300 w-96 rounded-lg">
        <h1 class="text-3xl text-slate-500 p-4">SignUp</h1>
        <form action="/users/store" method="post" class="px-5 w-full">
            @csrf
            <div class="-my-2">
                <label for="" class="inline-block text-xl mb-2">Name</label>
                <input type="text" name="name" class="border border-slate-500 rounded w-full py-2 px-3" placeholder="Enter your name" value="{{ old('name') }}" >
                <span class="text-red-500 text-sm @error('name') visible @else invisible @enderror">@error('name') {{$message}}@enderror <span class="invisible">A</span></span>
            </div>
            ...
            <div class="-my-2">
                <label for="" class="inline-block text-xl mb-2">Confirm password</label>
                <input type="password" name="password_confirmation" class="border border-slate-500 rounded w-full py-2 px-3" placeholder="Confirm your password">
                <span class="text-red-500 text-sm @error('password') visible @else invisible @enderror">@error('password') {{$message}}@enderror <span class="invisible">A</span></span>
            </div>
            <p class="text-slate-600">Already have an accout?
                <a href="/login" class="underline">Login</a> here
            </p>
